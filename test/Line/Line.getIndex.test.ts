...
describe('Line.prototype.qtRegions', () => {
    ...
    test('returns an array', () => {
        ...
        expect(Array.isArray(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 }))).toBe(true);
    });

    test('identifies quadrant top right', () => {
        const line = new Line({ x1: 75, y1: 25, x2: 80, y2: 30 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.TopRight);
    });

    test('identifies quadrant top left', () => {
        const line = new Line({ x1: 25, y1: 25, x2: 30, y2: 30 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.TopLeft);
    });

    test('identifies quadrant bottom left', () => {
        const line = new Line({ x1: 25, y1: 75, x2: 30, y2: 80 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.BottomLeft);
    });

    test('identifies quadrant bottom right', () => {
        const line = new Line({ x1: 75, y1: 75, x2: 80, y2: 80 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.BottomRight);
    });

    test('identifies overlapping top', () => {
        const line = new Line({ x1: 25, y1: 25, x2: 75, y2: 25 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.TopLeft | QuadRegions.TopRight);
    });

    test('identifies overlapping bottom', () => {
        const line = new Line({ x1: 25, y1: 75, x2: 75, y2: 75 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.BottomLeft | QuadRegions.BottomRight);
    });

    test('identifies overlapping left', () => {
        const line = new Line({ x1: 25, y1: 25, x2: 25, y2: 75 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.TopLeft | QuadRegions.BottomLeft);
    });

    test('identifies overlapping right', () => {
        const line = new Line({ x1: 75, y1: 25, x2: 75, y2: 75 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.TopRight | QuadRegions.BottomRight);
    });

    test('identifies diagonal /', () => {
        const line = new Line({ x1: 25, y1: 75, x2: 75, y2: 25 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.TopRight | QuadRegions.BottomLeft);
    });

    test('identifies diagonal \\', () => {
        const line = new Line({ x1: 25, y1: 25, x2: 75, y2: 75 });
        expect(line.qtRegions({ x: 0, y: 0, width: 100, height: 100 })).toEqual(QuadRegions.TopLeft | QuadRegions.BottomRight);
    });
    ...
    test('identifies edge', () => {
        ...
        expect(topLeft.qtRegions(node)).toEqual([1, 3]);
        expect(bottomRight.qtRegions(node)).toEqual([1, 3]);
    });
});